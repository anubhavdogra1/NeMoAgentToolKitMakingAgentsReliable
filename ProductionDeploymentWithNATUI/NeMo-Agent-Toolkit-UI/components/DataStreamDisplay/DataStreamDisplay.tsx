...
export const DataStreamDisplay: React.FC<DataStreamDisplayProps> = React.memo(
  ({ dataStreams, selectedStream, onStreamChange }) => {
      ...
      const interval = setInterval(async () => {
        try {
          ...
          const textRes = await fetch(
            `${HTTP_PROXY_PATH}${UPDATE_DATA_STREAM}?stream=${selectedStream}`,
          );
          if (textRes.ok) {
      ...
      const fetchLastDbUpdate = async () => {
        try {
          const response = await fetch(
            `${HTTP_PROXY_PATH}${UPDATE_DATA_STREAM}?type=finalized&stream=${selectedStream}`,
          );
          if (response.ok) {
            ...
            if (entries.length > 0) {
              ...
              const latestTimestamp = parseTimestampAsUTC(
        ...
        <div className="flex justify-between items-center mb-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {lastDbUpdate
                ? `Last DB update: ${formatLastUpdateTime(lastDbUpdate)}`
